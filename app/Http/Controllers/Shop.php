...
//
        ...
        View::share('products_hot', (new ShopProduct)->getProducts(2, 8, $opt = 'random'));
 ...
 */
    ...
    {
        $products = (new ShopProduct)->getProducts($type = 2, $limit = 12, $opt = 'random');
        if ($products) {
            return view($this->theme . '.shop_products',
                array(
                    'title_h1'    => 'Sản phẩm bán chạy -' . $this->configs['site_title'],
                    'title'       => 'Sản phẩm bán chạy -' . $this->configs['site_title'],
                    'description' => $this->configs['site_description'],
                    'keyword'     => $this->configs['site_keyword'],
                    'products'    => $products,
                    'search_enable' => false
 ...
 */
    ...
    public function postContact(Request $request)
    {
        ...
        try {
            ...
            ShopContact::insert(["name"=> $data["name"], "title"=> $data["title"]."\n"."Số lượng:". $data["quantity"], "content"=> $data["content"],"email"=> $data["email"],"phone"=> $data["phone"], "created_at" => \Carbon\Carbon::now()  ]);
    ...
    public function showDescription($id){
        $product = ShopProduct::find($id);
        $product_orther_images = $product->images;

        return response()->json([
            'product'=>$product,
            'images' =>$product_orther_images
        ]);
    }
